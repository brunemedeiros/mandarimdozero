...
- [ ] Skip link + labels de acessibilidade + contraste AA — **parcial, não marcado como concluído**: skip link existe e funciona (`<a class="skip-link" href="#main-content">`, alvo com `tabindex="-1"`); contraste de texto conferido nos pares principais (texto/fundo, texto secundário/fundo, cor de destaque/fundo, claro e escuro) — todos passam AA com folga (6.8:1 a 16:1, mínimo exigido é 4.5:1). Mas `aria-label` é raro no app do francês (só 3 ocorrências em `app.js` + 2 em `language-switcher.js`) — a maioria dos botões só-ícone (fechar ✕, tocar áudio 🔊, mudo, +15s/-15s, etc.) depende só de `title`, que não é confiável pra leitor de tela. Adicionar `aria-label` nesses botões é trabalho real (várias dezenas de botões pelo app inteiro) que não fiz por conta própria — fica pra quando for decidido fazer essa passada.
