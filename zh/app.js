...
function playPregeneratedAudio(file, btnEl, isAutoplay){
  ...
  audio.play().catch(() => {
    ...
    // gesto do usuário que o play() automático depende) -- diferente de um
    // erro real de arquivo/rede, por
...
const STATE = {
  ...
  // fora do escopo da 6C.2. `hanziReviewShowingAnswer` (abaixo) é de
  // OUTRA feature (revisão de hanzi), não tocado aqui.
...
// permitir rastrear erro por palavra (ver showCorrectFeedbackPanel/showAnswerPanel).
...
// não uma representação do XP real (lessonStars/registerDailyStars/desafio
// 'stars40' removidos na Fase 6 -- ver GENERAL_CHALLENGES/addXP).
// trackHistory=false (usado pelo checkpoint, == isCheckpoint aqui) pula o
...
// formatos onde a resposta certa já fica visível na própria tela do
// exercício antes do painel abrir (múltipla escolha destaca a opção certa,
...
// branco). Cartões cloze já existentes continuam com back_hanzi
...
// distinção hanzi revelado/pinyin comparado de sempre, só a
// leitura/escrita de "já respondeu?" muda de STATE pra localState.
//
